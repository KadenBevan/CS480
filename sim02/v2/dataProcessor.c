...
int parse_data(char* fn_config, ConfigData* config_data, LinkedList* list)
{
	...
	if (extract_config(fn_config, config_data) < 0)
	{
		return CONFIG_EXTRACT_ERROR;
	}
	if (extract_meta_data(config_data->metaFilePath, list) < 0)
	{
		return META_EXTRACT_ERROR;
	}
	return 0;
}
...
int extract_config(char* fn_config, ConfigData* config_data)
{
	...
	FILE* ptr_file;
	if ((ptr_file = fopen(fn_config, "r")) == NULL)
	{
...
int extract_meta_data(char* fp_meta_data, LinkedList* list)
{
	// open file
	FILE* ptr_file;
	if ((ptr_file = fopen(fp_meta_data, "r")) == NULL)
	{
		return OPEN_FILE_ERROR;
	}
	// get string line
